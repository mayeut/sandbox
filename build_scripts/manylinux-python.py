...
NO_CHECK = os.environ.get("MANYLINUX_PYTHON_NO_CHECK", "0") == "1"
...
def add_parser_list(subparsers):
    ...
    parser.add_argument("-i", "--installed", default=False, action="store_true")
...
def _list(args):
    ...
    for tag in tags:
        print(f"{tag}{':' if args.verbose else ''}")
        if args.verbose:
            ...
            print(f"  python version:       {info['pv']}")
            print(f"  implemention:         {info['i']}")
            print(f"  implemention version: {info['iv']}")


def add_parser_ensure(subparsers):
    parser = subparsers.add_parser("ensure")
    parser.set_defaults(func=ensure)
    parser.add_argument("tags", choices=get_all_tags(no_check=NO_CHECK), metavar="TAG", nargs='+')
...
def ensure_one(tag):
    ...
    install_script = HERE / "download-and-install-python.sh"
...
def add_parser_ensure_all(subparsers):
    parser = subparsers.add_parser("ensure-all")
