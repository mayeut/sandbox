...
# create manylinux-python script
cat <<EOF > /usr/local/bin/manylinux-python
#!/bin/bash
...
/opt/python/cp310-cp310/bin/python $MY_DIR/manylinux-python.py "\$@"
EOF
chmod 755 /usr/local/bin/manylinux-python

MANYLINUX_PYTHON_NO_CHECK=1 /usr/local/bin/manylinux-python ensure "$@"
